...
int AddSetToSet(Set *pDesSet, const Set *pSrcSet) {
    ...
    for (int i = 0; i < pSrcSet->nTerminalCount; i++) {
        if (strcmp(pSrcSet->Terminal[i], VoidSymbol) != 0 &&
            AddTerminalToSet(pDesSet, pSrcSet->Terminal[i])) {
            flag = 1;
        }
    }
    return flag;
...
参数：
        ...
        pFirstSetList -- First 集合指针。
*/
void Follow(const Rule *pHead, SetList *pFollowSetList,
    ...
    do {
        ...
        for (pRule = pHead; pRule != NULL; pRule = pRule->pNextRule) {
            for (pSymbol = pRule->pFirstSymbol; pSymbol != NULL;
                 pSymbol = pSymbol->pNextSymbol) {
                // 忽略终结符
                ...
                if (NULL != pSymbol->pNextSymbol) {
                    ...
                    } else  // 非终结符
                    {
                        ...
                        Set *pFirstSetXn = GetSet(
                            pFirstSetList, pSymbol->pNextSymbol->SymbolName);

                        // 将找到的 Set 加入 TempFirstSet 中
                        AddSetToSet(&TempFirstSet, pFirstSetXn);
                    }
                } else {
                    // 调用 AddTerminalToSet 函数将 VoidSymbol 加入到
                    // TempFirstSet 中
                ...
                if (SetHasVoid(
                        &TempFirstSet))  // 判断集合 TempFirstSet 是否含有ε
                {
